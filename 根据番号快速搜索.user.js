...
(function() {
    ...
    var oRegExp = /(?<!\w|\/|www\.|col-)(?!heyzo|SHINKI|JPNXXX|carib)[a-zA-Z]{2,6}-\d{2,5}(?:-c|_c|-4k)?(?!\d|[A-Za-z]{2,}|[0-9]|-\d|\.com)|(?<!\w|\/|\.|#|@|www\.)(?!heyzo|SHINKI|JPNXXX|carib|and)[a-zA-Z]{2,6}\s?\d{3,4}(?:-c|_c)?(?!\d|[A-Za-z]|[0-9]|-|\.|％|%|@|歳| 歲|分| Min| day| time|cm| ppi|\.com)|(?<!\w)(?:PARATHD|3DSVR)[-\s]?\d{3,4}(?!\w)|(?<!\w)(?:HIMEMIX|CASMANI)[-\s]?\d{3}(?!\w)|(?<!\w|\d-)\d{6}_(?:1)?\d{3}_0[12](?!\w|-\d)/gi; 
    ...
    function addEventAndStyle(isExist){
        ...
        var rule = {};
        if(isExist){
            rule = setting.visitStyle ? setting.visitStyle:{"textDecoration":"underline dotted #66ccff"}
        } else {
            rule = setting.linkStyle ? setting.linkStyle:{"textDecoration":"underline green"}
        }

        if(rule){
            for(let key in rule){
                odiv.style[key] = rule[key]
            }
        }
        return odiv;
    }
    ...
    function avmouseenter(e){
        ...
        var rule = setting.visitStyle ? setting.visitStyle:{"textDecoration":"underline dotted #66ccff"}
        for(let key in rule){
            e.target.style[key] = rule[key]
        }
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (data) {
                ...
                if(data.status==404 && avID.length - avID.indexOf("-") ==3){
                    ...
                    avInfo.noInfo = true
                    localInfo[avID] = {};   
                    localInfo[avID].noInfo = avInfo.noInfo;
                ...
                if(data.status==404 && avID.match(oRegExp_SuRen)){
            ...
            onload: function (data) {
                ...
                for(let i=0;i<other.length;i++){
                    if(other[i].innerHTML.search("日期:")>-1){
                        avInfo.date = other[i].innerText.replace(/\s{2,}/g,"")  //trim();
                    }
    ...
    function formatWuma(otext){
        ...
        if(otext.match(/heyzo/i)){  
            ...
            otext = "HEYZO-" + otext.slice(oindex)
        }
        // 下划线是加勒比,
        // 横杠是 pacopacomama
        // // カリビアンコム 加勒比
        ...
        if(otext.match(/HEYDOUGA/i)){
            otext = "heydouga-" + otext.slice(otext.search(/\d/i))
        }
    ...
    function savDebug(){
        ...
        var debug_setting = {
            ...
            "dontAutoClearMenu": false, // 鼠标移出后,菜单不会消失(测试时找问题使用, 开启会影响脚本使用)
            "linkStyle":{   // 没浏览的番号
                "color":"red",  // 颜色
                "text-decoration":"underline red",  //下划线
                "font-weight":"bold",   // 加粗 normal、bold、lighter 等
                "text-shadow":"1px 1px #478747",    // 字体阴影
            },
            "visitStyle":{  // 已经浏览过的番号
                "color":"green",
                "text-decoration":"underline green",
                "font-weight":"bold",
                "text-shadow":"1px 1px #cd9595",
            },
        }
